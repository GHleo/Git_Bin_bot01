            ...
            for order in orders_info:
                ...
                if orders_info[order]['order_type'] == 'buy':
                    ...
                    if order_status == 'FILLED':
                        ...
                        need_to_earn = orders_info[order]['buy_amount'] * orders_info[order]['buy_price'] * (
                                    1 + all_pairs[stock_order_data['symbol']]['profit_markup'])
                        ...
                        """.format(
                            buy_initial=orders_info[order]['buy_amount'], has_amount=has_amount,
                ...
                if all_pairs[orders_info[order]['order_pair']]['use_stop_loss']:
                    ...
                    if order_status == 'NEW' and orders_info[order]['order_type'] == 'sell':
                        ...
                        if (1 - curr_rate / orders_info[order]['buy_price']) * 100 >= \
                                all_pairs[orders_info[order]['order_pair']]['stop_loss']:
                            log.debug(
                                "{pair} Цена упала до стоплосс (покупали по {b:0.8f}, сейчас {s:0.8f}), пора продавать".format(
                            ...
                            log.debug(
                                "{pair} (покупали по {b:0.8f}, сейчас {s:0.8f}), расхождение {sl:0.4f}%, panic_sell = {ps:0.4f}% ({ps_rate:0.8f}), продажа с профитом: {tp:0.8f}".format(
                                    pair=orders_info[order]['order_pair'],
                                    b=orders_info[order]['buy_price'],
                                    s=curr_rate,
                                    sl=(1 - curr_rate / orders_info[order]['buy_price']) * 100,
                                    ps=all_pairs[orders_info[order]['order_pair']]['stop_loss'],
                                    ps_rate=orders_info[order]['buy_price'] / 100 * (
                                                100 - all_pairs[orders_info[order]['order_pair']]['stop_loss']),
            ...
            for pair_name, pair_obj in all_pairs.items():
                ...
                if balances[pair_obj['base']] >= pair_obj['spend_sum']:
                    ...
                    try:
                        ...
                        if my_amount < float(CURR_LIMITS['filters'][1]['stepSize']) or my_amount < float(
                                CURR_LIMITS['filters'][1]['minQty']):
                            log.warning("""
                        ...
                        log.debug(
                            'Рассчитан ордер на покупку: кол-во {amount:0.8f}, курс: {rate:0.8f}'.format(
